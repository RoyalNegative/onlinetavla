  ...
  return (
    <div className={`flex items-center gap-3 rounded-xl px-3 py-2 transition ${active ? 'bg-accent/15 ring-1 ring-accent/50' : 'bg-cream/5'}`}>
      ...
      <div className="min-w-0 flex-1">
        ...
        <span className="text-xs text-cream/50">⚓ {board.shipsLeft} gemi ayakta</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="card p-3">
        <div className="mb-2 px-1 text-xs text-cream/50">Amiral Battı · {view.noTouch ? 'Kolay mod' : 'Zor mod'}</div>
      ...
      <div className="card space-y-3 p-4">
        {over ? (
          ...
          <>
            <p className="text-center text-sm text-cream/70">{status}</p>
